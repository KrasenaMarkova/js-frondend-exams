...
function solve() {
  function createElement(tag, properties, container) {
    ...
    // минаваме през всичките ключове на properties, които искаме да закачим на обекта
    Object.keys(properties).forEach(key => {
      // ако текущият ключ е object
      if (typeof properties[key] === 'object') {
        // копираме Object елементите с техните ключове и стойности от един обект на друг
    ...
    // ако сме подали нещо в container, да закаччим element
    if (container) container.append(element);

    // връщаме референцията към елемента, ако ни се наложи да правим нещо с него
    return element;
  }
  ...
  function createEntry({ name, time, desc }) {
    ...
    // dataset начин, по който можем да достъпваме атрибутите на даден елемент
